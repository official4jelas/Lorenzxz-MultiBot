...
const plugins = loadPlugins(path.resolve(__dirname, "../plugins/discord"))
...
if (!global.discord || !global.discord.token || global.discord.token.trim() === "") {
} else {
    try {
        client.login(global.discord.token)
    } catch (error) {}
}